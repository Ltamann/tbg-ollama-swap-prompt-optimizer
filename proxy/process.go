...
func (p *Process) SetRuntimeCtxSize(ctxSize int) {
	if ctxSize <= 0 {
		p.runtimeCtxSize.Store(0)
		return
	}
...
func applyCtxSizeArg(args []string, ctxSize int) []string {
	...
	for i := 0; i < len(updated); i++ {
		...
		if arg == "--ctx-size" || arg == "-c" {
			if i+1 < len(updated) {
				updated[i+1] = ctxValue
				return updated
			}
			updated = append(updated, ctxValue)
			return updated
		}

		if strings.HasPrefix(arg, "--ctx-size=") {
			updated[i] = "--ctx-size=" + ctxValue
			return updated
		}
	}
